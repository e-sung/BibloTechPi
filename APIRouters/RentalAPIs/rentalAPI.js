...
router.patch('/status-of/book/:id/to/:situation',(req,res)=>{
	console.log("rental api called")
	if(req.params.situation === "rent"){
		rentTheBookWith(req,res)
	}else if(req.params.situation === "return"){
...
function returnTheBookWith(req,res){
	...
	.then(function queryUserInfo(){
		...
		var sql3 = mysql.format("select readBooks from user where email = ?", userEmail)
		return db.query(sql3)
	})
	.then(function reward(user){
		...
		if(alreadyRead){//if user read this book already, just update rentable books and rentscore
			sql4 = "update user set rentableBooks = rentableBooks + 1, rentscore = rentscore + 10 where email = ?"
			inserts = [userEmail]
		}else{//if user hasn't read htis book, update everything
			sql4 = "update user set rentableBooks = rentableBooks + 1, rentscore = rentscore + 10, readBooks = concat(?,readBooks) where email = ?"
			inserts = [bookTitle + ";", userEmail]					
		}
